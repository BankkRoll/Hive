...
function DropdownMenuTrigger({ asChild, children, ...props }: DropdownMenuTriggerProps) {
  // When asChild is true, render as the child element instead of default button
...
interface DropdownMenuItemProps extends Omit<MenuPrimitive.Item.Props, "render"> {
  inset?: boolean;
  ...
  if (asChild && React.isValidElement(children)) {
    return (
      <MenuPrimitive.Item
        ...
        render={React.cloneElement(children as React.ReactElement<{ className?: string }>, {
          className: cn(
            itemClassName,
